...
io.on('connection', async (socket) => {
  ...
  try {
     ...
     */
    socket.on('create_conversation', async ({ participantIds, isGroup, groupName }) => {
      try {
        ...
        await conversation.populate('participants', 'username imageUrl isOnline');

        allParticipants.forEach(participantId => {
          io.to(`user:${participantId}`).emit('conversation_created', conversation);
        });
 ...
 */
app.get('/api/conversations/:conversationId/messages', verifyClerkToken, async (req, res) => {
  try {
